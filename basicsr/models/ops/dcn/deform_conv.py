...
class ModulatedDeformConvPack_WFlow(ModulatedDeformConv):
    """A ModulatedDeformable Conv Encapsulation that acts as normal Conv layers.
    ...
    def __init__(self, *args, **kwargs):
        super(ModulatedDeformConvPack_WFlow, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(ModulatedDeformConvPack_WFlow, self).init_weights()
        if hasattr(self, 'conv_offset'):
            self.conv_offset.weight.data.zero_()
            self.conv_offset.bias.data.zero_()

    def forward(self, x, feat, flow=None):
